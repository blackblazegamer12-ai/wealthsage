    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            {/* Header */}
            <div className="px-6 py-5 border-b flex items-center justify-between backdrop-blur-xl" style={{ backgroundColor: 'var(--bg)', borderColor: 'var(--border-subtle)' }}>
                      ...
                      {isUser ? (
                        ...
                        <div className="prose prose-invert prose-sm max-w-none prose-p:leading-relaxed">
                          <ReactMarkdown
                            ...
                            components={{
                              h1: ({node, ...props}) => <h1 style={{color: 'var(--text-primary)', fontSize: '1.25rem', fontWeight: 'bold'}} {...props}/>,
                              h2: ({node, ...props}) => <h2 style={{color: 'var(--text-primary)', fontSize: '1.125rem', fontWeight: 'bold'}} {...props}/>,
                              h3: ({node, ...props}) => <h3 style={{color: 'var(--text-primary)', fontSize: '1rem', fontWeight: 'bold'}} {...props}/>,
                              strong: ({node, ...props}) => <strong style={{color: 'var(--accent)'}} {...props}/>,
                              a: ({node, ...props}) => <a style={{color: 'var(--accent)'}} {...props}/>,
                              code: ({node, ...props}) => <code style={{color: 'var(--accent)', backgroundColor: 'var(--surface-overlay)', padding: '2px 4px', borderRadius: '4px'}} {...props}/>,
                            }}
                          >
                            {msg.message}
                          </ReactMarkdown>
            ...
            {/* Quick Prompts */}
            <div className="px-4 py-3 border-t overflow-x-auto no-scrollbar flex items-center gap-2" style={{ borderColor: 'var(--border-subtle)', backgroundColor: 'var(--bg)' }}>
              {quickPills.map((qp) => (
                <button
                  key={qp.label}
                  type="button"
            ...
            {/* Input Area */}
            <div className="p-4 border-t" style={{ borderColor: 'var(--border-subtle)', backgroundColor: 'var(--bg)' }}>
